...
class Wc_Smart_Cod_Public {
		...
		foreach( $this->cod_settings as $k => $v ) {
			...
			if( ( $k === 'extra_fee' || $k === 'nocharge_amount' || strpos( $k, 'different_charge' ) !== false ) && is_numeric( $v ) ) {
				...
				if( $k === 'extra_fee' ) {
					...
					$fee_table[ $k ][ 'restriction' ] = $this->nocharge_amount_mode;
	...
	public function apply_custom_message( $data ) {
		...
		if( $this->has_cod_available() === false ) {
			...
			if( $message ) {
				...
				$first_element = $doc->getElementsByTagName( 'body' )->item( 0 )->childNodes->item( 0 )->childNodes->item( 0 );
	...
	private function get_customer_shipping_method( $inside_zone = false ) {
		...
		if( $id === false ) {
			...
			$chosen_rate = WC()->session->get( 'chosen_shipping_methods' );
			$id = $this->get_method_id( $chosen_rate[ 0 ], $inside_zone, $packages );
		}
	...
	public function has_cod_available() {
		...
		// check native gateway
		// method restriction

		$has_cod_available = $this->check_method_restriction();
		if( ! $has_cod_available ) {
			$this->reason = 'shipping_method_restriction';
		}
