...
<form id="yith-wcwl-form" action="<?php echo $form_action ?>" method="post" class="woocommerce">
    ...
    outline: none;
    background: url(../../Product/img/cart.png) #94cb39 no-repeat left 16px center;
    ...
    text-align: center;
                    ...
                    <?php 
                    $sale_info = "";
                    $sale_link =get_field('link_sale',$item['prod_id']);
                    ?>
                    <td>
                                <?php
                                    if($sale_link){
                                ?>
                                <a href="<?=$sale_link;  ?>"><img src="https://sanhangnhapkhau.com/wp-content/uploads/2018/06/Sale-tag-Icon-724820.png" style="width:50px;height:auto"/></a>
                                <?php } ?>
                    </td>
                    </tr>
                <?php
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count+1 ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist', 'yith-woocommerce-wishlist' ) ) ?></td>
            </tr>
        <?php
        endif;
        ...
        if( ! empty( $page_links ) ) : ?>
            <tr class="pagination-row">
                <td colspan="<?php echo esc_attr( $column_count+1 ) ?>"><?php echo $page_links ?></td>
            </tr>
        <?php endif ?>
        </tbody>
        ...
        <tfoot>
        <tr>
	        <td colspan="<?php echo esc_attr( $column_count+1 ) ?>">
	            <?php if( $show_cb ) : ?>
